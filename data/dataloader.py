...
class LoRADataset(Dataset):
    ...
    def _load_data(self) -> List[Dict[str, torch.Tensor]]:
        ...
        # 处理数据是列表，每四个元素作为一组参数的情况
        if isinstance(raw_data, list) and len(raw_data) >= 4 and not isinstance(raw_data[0], (dict, list, tuple)):
            # 每四个元素为一组
            for i in range(0, len(raw_data), 4):
                if i + 3 < len(raw_data):  # 确保有完整的四个元素
                    try:
                        processed_item = {
                            'lora1_a': torch.FloatTensor(raw_data[i]),
                            'lora1_b': torch.FloatTensor(raw_data[i+1]),
                            'lora2_a': torch.FloatTensor(raw_data[i+2]),
                            'lora2_b': torch.FloatTensor(raw_data[i+3])
                        }
                        processed_data.append(processed_item)
                    except Exception as e:
                        logger.warning(f"Error processing group starting at index {i}: {e}, skipping")
                        continue
            
            logger.info(f"Processed {len(processed_data)} groups from list data (4 elements per group)")
